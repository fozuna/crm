...
CREATE TABLE IF NOT EXISTS financial_accounts_receivable (
  ...
  INDEX idx_financial_receivable_service_order (service_order_id),
  ...
  CONSTRAINT fk_financial_receivable_service_order FOREIGN KEY (service_order_id) REFERENCES servicos_avulsos(id) ON DELETE SET NULL,
...
-- ============================================================================
-- BLOCO 8: MODULO DE ORDENS DE SERVICO.
-- Estrutura independente para registrar demandas pontuais, anexos, historico
-- operacional e vinculo opcional com o financeiro do CRM.
-- ============================================================================
...
INSERT INTO financial_categories (company_id, name, type, color, active, created_at, updated_at)
SELECT 1, 'Serviços avulsos', 'receivable', '#8B5CF6', 1, NOW(), NOW()
WHERE NOT EXISTS (
  SELECT 1
  FROM financial_categories
  WHERE company_id = 1 AND name = 'Serviços avulsos'
);

INSERT INTO financial_cost_centers (company_id, name, code, active, created_at, updated_at)
SELECT 1, 'Serviços Avulsos', 'OS', 1, NOW(), NOW()
WHERE NOT EXISTS (
  SELECT 1
